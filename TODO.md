...
最新更新：2026-06-09
目前階段：Phase 5 進行中（pipeline + routine + idempotent 完成，待 7 天連續穩定驗證）
...
- [x] `config/feeds.yaml`（32 sources，27 enabled）
- [x] `config/beats.yaml`（4 大 beat 關鍵字 + entity 設定 + TW Highlight + AI/ECON 例外）
...
- [x] `tests/test_fetch_rss.py`（12 條）+ `tests/test_normalize.py`（26 條）
- [x] 實機驗證：BBC World 抓 32/36 articles，schema 全過

## Done — Phase 2 ✅

- [x] `src/classifier.py` — Beat 分類（source 0.6 + keyword 0.3 + entity 0.1）+ AI/ECON 例外
...
- [x] `tests/test_classifier.py`(22) + `test_entity_recognizer.py`(19) + `test_tw_highlight.py`(10) + `test_dedup.py`(11) + `test_event_cluster.py`(24) + `test_selector.py`(18) — Phase 2 模組測試
...
- [x] `tests/test_html_cleaner.py`(21) + `test_validators.py`(37) + `test_claim_trace.py`(18) + `test_claude_rewrite.py`(23) — Phase 3 模組測試（mocked SDK）
- [x] `docs/INTEGRATION_DESIGN.md` — akasha-rss-news ↔ akasha-library 整合設計稿

**Phase 3 完成時測試：250 條全綠**

## Done — Phase 4 ✅

- [x] `src/formatter.py` — 多格式輸出（JSON / Markdown / Voice / X / Threads + run log）
- [x] `tests/test_formatter.py`(66) — split_posts / platform_output / report / markdown / voice / drafts / validation / integration
- [x] 驗收：voice_text lint + X ≤280 + Threads ≤500 + 朗讀版獨立生成 + Markdown 多 beat 排版

**Phase 4 完成時測試：306 條全綠**

---

## Now — Phase 5 進行中

**測試合計：366 條全綠（含 Data Layer Audit +18 條、品質改進 +42 條）**

Routine 自動化 + 7 天穩定性。

- [x] 寫 `src/pipeline.py`（串接 Phase 1–4 所有模組的 main 入口）+ 21 條測試
...
- [ ] 通知管道設定（月月決定先不做，之後再加）
- [x] 同日重跑 idempotent 測試（2 條：summary 一致 + 檔案覆寫不重複）
- [ ] 連跑 7 天穩定性測試（routine 已連續產出 6 天：05-30~06-04）
